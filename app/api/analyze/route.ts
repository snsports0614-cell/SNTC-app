...
export async function POST(req: NextRequest) {
  try {
    const apiKey = 'AIzaSyD_AqArJ1Imy1ve0EB3uXUqOpMkQ1IZ1iE';
    if (!apiKey) return NextResponse.json({ error: 'API 키가 없습니다.' });
    ...
    const systemPrompt = `
      ...
      [응답 형식 (JSON)]
      {
        "overallReview": "3줄 이내의 짧고 강렬한 총평",
        "testResults": [
          {
            "testName": "항목명",
            "left": 0,
